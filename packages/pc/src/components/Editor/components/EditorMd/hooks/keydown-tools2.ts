...
function _getParagraphInfo(value: string, index: number): ParagraphInfo {
...
export function getParagraphContext(value: string, startIndex: number): ParagraphContext {
  const p = _getParagraphInfo(value, startIndex)
  const pUp = _getParagraphInfo(value, p.startIndex - 1)
  const pDown = _getParagraphInfo(value, p.endIndex + 1)
  return {
    p,
    pUp,
    pDown
