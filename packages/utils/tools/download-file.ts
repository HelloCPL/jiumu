 ...
 * 请求返回文本
 */
export const getFileText = (url: string, unicode: string = 'utf-8'): Promise<any> => {
  return new Promise((resolve) => {
 ...
 * 请求返回 blob 文件流
 */
export const getFileBlod = (url: string, headers: ObjectAny = {}): Promise<any> => {
  return new Promise((resolve) => {
    ...
    const user = storage.getItem('User', { prefix: 'pinia', type: 'local' })
    if (user?.token) {
