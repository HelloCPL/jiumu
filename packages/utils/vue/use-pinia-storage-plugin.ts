...
import { StorageOption, storage } from '../tools/storage'
const { VITE_HOME_EXPIRE, VITE_SESSION_EXPIRE } = import.meta.env
...
export const usePiniaStoragePlugin = ({ store, options }: PiniaPluginContext) => {
  if (options.storage?.enabled) {
    ...
    const type = options.storage?.type || 'local'
    const expire =
      options.storage?.expire ?? type === 'local' ? VITE_SESSION_EXPIRE * 1 : VITE_HOME_EXPIRE * 1
